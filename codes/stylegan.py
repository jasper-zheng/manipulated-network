...
from NetworkOperations import Scale,Block,RepeatX,RepeatY,Shuffle,Sin_disrupt,Brightness,Rotate,Translate,Invert
...
def StyleGAN_G_synthesis(dlatent_size=512, resolution=1024, clusters=[], operations=[]):
    # General parameters
    num_channels = 3
    resolution_log2 = int(np.log2(resolution)) #1024:10
    ...
    def process_params(layer):
        if not 'scale' in layer:
            layer['scale']=0
        if not 'delta' in layer:
            layer['delta']=0
        if not 'translateX' in layer:
            layer['translateX']=0
        if not 'translateY' in layer:
            layer['translateY']=0
        if not 'filter_shape' in layer:
            layer['filter_shape']=3
        if not 'filter_shape' in layer:
            layer['filter_shape']=3
        if not 'rotation' in layer:
            layer['rotation']=0
        if not 'sharpen_factor' in layer:
            layer['sharpen_factor']=1
        if not 'clusters' in layer:
            layer['clusters']=[]
        if not 'sinX' in layer:
            layer['sinX']=[1]
        if not 'sinY' in layer:
            layer['sinY']=[1]
        return layer
        
    # Things to do at the end of each layer.
    def layer_epilogue(x, layer_idx, name, res=0, cluster = [], block_sequence=[]):
        ...
        if block_sequence:
            block_sequence = block_sequence[0]
        else:
            #print('norm')
            pass      
        
        for i, layer in enumerate(block_sequence):
            if not layer['operation'] == 'none':
                layer = process_params(layer)
                
                if layer['clusters']:
                    seq = np.array( [1 if l in layer['clusters'] else 0 for l in cluster] )
                    #print(f'mask installed to {layer["clusters"]}')
                else:
                    seq = np.ones((512,))
                    
                this_name = f'{layer["operation"]}/{x.shape[2]}x{x.shape[2]}/{layer["name"]}_{i}'
                x = Block(name=this_name,
                          sequences=seq,
                          ...
                          sharpen_factor=layer['sharpen_factor'],
                          sinX=layer['sinX'],
                          sinY=layer['sinY'])(x)
                if layer['with_norm']:
                    #print('added norm')
                    x = InstanceNorm(name=f'InstanceNorm/{this_name}')(x)
    ...
    def block(res, x, clusters=[], operations=[]): # res = 3..resolution_log2
        this_resolution = 2**res
        name, name0, name1 = '%dx%d' % (this_resolution, this_resolution), 'Conv0_up', 'Conv1'
        ...
        cluster_second = []
        if clusters:
            cluster_first = clusters[(res-1)*2-2]
            cluster_second = clusters[(res-1)*2-1]
        ...
        x = layer_epilogue( Blur(name='G_synthesis/{}/{}/Blur'.format(name, name0))(upscaled), 
                           ...
                           block_sequence = [op['layers'] for op in operations if op['resolution']==this_resolution and op['install_after']=='Conv0_up'])
        ...
        x = layer_epilogue( Conv2d(name='G_synthesis/{}/{}'.format(name, name1), filters=nf(res-1), kernel_size=3, use_bias=False)(x), 
                           ...
                           block_sequence = [op['layers'] for op in operations if op['resolution']==this_resolution and op['install_after']=='Conv1'])
    ...
    cluster_second = []
    if clusters:
        cluster_first = clusters[0]
        cluster_second = clusters[1]
    # Early layers.
    x = layer_epilogue(Const(name='G_synthesis/4x4/Const')(dlatents_in), 
                       0, 
                       name='Const',
                       res=1,
                       cluster = cluster_first,
                       block_sequence = [op['layers'] for op in operations if op['resolution']==4 and op['install_after']=='Conv0_up'])
    x = layer_epilogue(Conv2d(name='G_synthesis/4x4/Conv', filters=nf(1), kernel_size=3, use_bias=False)(x), 
                       1, 
                       name='Conv',
                       res = 2,
                       cluster = cluster_second,
                       block_sequence = [op['layers'] for op in operations if op['resolution']==4 and op['install_after']=='Conv1'])
    ...
    for res in range(3, resolution_log2 + 1):
        #res: 3 - 9
        x = block(res, x, clusters=clusters, operations=operations)
...
class StyleGAN_G(Model):
    def __init__(self, resolution=1024, latent_size=512, dlatent_size=512, mapping_layers=8, mapping_fmaps=512, mapping_lrmul=0.01, clusters=[], operations=[] ):
        ...
        self.model_synthesis = StyleGAN_G_synthesis(dlatent_size, resolution, clusters, operations)
        print('Model created.')
    ...
    def rebuild_operations(self,operations,clusters=[]):
        self.model_synthesis = StyleGAN_G_synthesis(self.dlatent_size, self.resolution, clusters, operations)
        copy_weights_to_keras_model(self.model_synthesis, self.all_weights)
        print('Synthesis network rebuilt and loaded.')
    ...
    def generate_from_vector_fast(self,latents,is_visualize=True):
        y = self.model_mapping(latents)
        y = self.model_synthesis.predict(y)
        images = y.transpose([0, 2, 3, 1])
        images = np.clip((images+1)*0.5, 0, 1)
        print(images.shape, np.min(images), np.max(images))
        if is_visualize:
    ...
    def generate_from_inter(self,latents,is_visualize=True):
        #y = self.model_mapping(latents)
        y = self.model_synthesis.predict(latents)
        images = y.transpose([0, 2, 3, 1])
        images = np.clip((images+1)*0.5, 0, 1)
        print(images.shape, np.min(images), np.max(images))
        if is_visualize:
...
def copy_weights_to_keras_model(model, all_weights, console=False):
    c = 0
    od = all_weights
