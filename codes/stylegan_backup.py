...
from tensorflow.keras.layers import Layer, InputLayer, Multiply, Lambda, Flatten, Dense, Conv2D, Conv2DTranspose,BatchNormalization
...
class Translate(tf.keras.layers.Layer):
  ...
  def call(self, inputs):
    ...
    rot = tfa.image.translate(trans_1,[2,0], interpolation='nearest', fill_mode='reflect')
...
def StyleGAN_G_synthesis(dlatent_size=512, resolution=1024):
    ...
    def layer_epilogue(x, layer_idx, name, res=0):
    ...
    def block(res, x): # res = 3..resolution_log2
        ...
        x = layer_epilogue( Blur(name='G_synthesis/{}/{}/Blur'.format(name, name0))(upscaled), res*2-4, name0 ,res = res)
